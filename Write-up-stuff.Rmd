...
```{r}
lh_stats <- left %>% group_by(pitch_name) %>% 
  summarize(release_speed = mean(release_speed), 
            release_spin = mean(release_spin_rate),
            hor_break = mean(hor_break), 
            vert_break = mean(ind_vert_break), 
...
```{r}
right_curve <- right %>% filter(pitch_name == "Curveball")
...
right_curve <- right_curve %>% 
  mutate(ideal_prob = (exp(0.0311 + 0.0121*hor_break + 0.0004*mean_centered_spinrate)
                       / (1+exp(0.0311 + 0.0121*hor_break + 0.0004*mean_centered_spinrate))))
