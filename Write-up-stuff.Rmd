...
pitcher_ideal_creation <- function(dataset) {
  dataset <- dataset %>% mutate(pitcher_ideal = if_else(description == "called_strike" | description == "foul"| description == "foul_bunt"| description == "foul_tip"| description == "missed_bunt"| description == "swinging_strike"| description == "swinging_strike_blocked" | estimated_ba_using_speedangle <= .9 , 1, 0))
...
rh_breakingball <- rh_breakingball %>% 
  mutate(ideal_prob = (exp(.537) * exp(ind_vert_break*0.015)))
...
rh_fourseam <- rh_fourseam %>% 
  mutate(ideal_prob = (exp(0.230) * exp(ind_vert_break*0.028)))
...
```{r}
rh_fc_model_final <- glm(pitcher_ideal ~ release_speed, family = "binomial", 
                     data = rh_cutter)

tidy(rh_fc_model_final) %>% kable(digits = 3)
```

```{r}
rh_fc_model_intercept <- glm(pitcher_ideal ~ 1, family = "binomial", 
                     data = rh_cutter)

kable(anova(rh_fc_model_final, rh_fc_model_intercept, test = "Chisq"))
...
#### LH Cutter
```{r}
lh_cutter <- rh_fastball %>% filter(pitch_type == "FC") %>% group_by(player_name) %>% 
  mutate(mean_centered_axis = mean(spin_axis) - spin_axis)

lh_fc_model <- glmer(pitcher_ideal ~ release_speed  + hor_break + ind_vert_break + 
                       mean_centered_axis + (1|player_name), family = "binomial", 
                     data = lh_cutter)
...
```{r}
lh_fc_model_final <- glm(pitcher_ideal ~ release_speed, family = "binomial", 
                     data = lh_cutter)

tidy(lh_fc_model_final) %>% kable(digits = 3)
```
