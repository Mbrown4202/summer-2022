...
```{r inch-switch}
lefty_ninth <- lefty_ninth %>% drop_na(release_speed, release_spin_rate, pfx_x, 
                        pfx_z, spin_axis)
lefty_ninth <- lefty_ninth %>% mutate(hor_break = (pfx_x*12), 
                                      ind_vert_break = (pfx_z*12))

righty_ninth <- righty_ninth %>% drop_na(release_speed, release_spin_rate, pfx_x, 
                        pfx_z, spin_axis)
...
```{r}
rhf_stats <- rh_fastball %>% group_by(pitch_name) %>% 
...
```{r}
lhf_stats <- lh_fastball  %>% group_by(pitch_name) %>% 
  summarize(release_speed = mean(release_speed), 
            release_spin = mean(release_spin_rate),
            hor_break = mean(hor_break), 
            vert_break = mean(ind_vert_break), 
...
```{r}
rhb_stats <- rh_breakingball %>% group_by(pitch_name) %>% 
  summarize(release_speed = mean(release_speed), 
            release_spin = mean(release_spin_rate),
            hor_break = mean(hor_break), 
            vert_break = mean(ind_vert_break), 
...
```{r}
lhb_stats <- lh_breakingball %>% group_by(pitch_name) %>% 
  summarize(release_speed = mean(release_speed), 
            release_spin = mean(release_spin_rate),
            hor_break = mean(hor_break), 
            vert_break = mean(ind_vert_break), 
...
```{r}
rhc_stats <- rh_changeup %>% group_by(pitch_name) %>% 
  summarize(release_speed = mean(release_speed), 
            release_spin = mean(release_spin_rate),
            hor_break = mean(hor_break), 
            vert_break = mean(ind_vert_break), 
...
```{r}
lhc_stats <- lh_changeup %>% group_by(pitch_name) %>% 
  summarize(release_speed = mean(release_speed), 
            release_spin = mean(release_spin_rate),
            hor_break = mean(hor_break), 
            vert_break = mean(ind_vert_break), 
...
rh_bb_model <- glm(pitcher_ideal ~ release_speed + hor_break + 
                       ind_vert_break + mean_centered_spinrate,
...
rh_bb_model_final <- glm(pitcher_ideal ~
                       ind_vert_break + mean_centered_spinrate, 
                     family = "binomial", data = rh_breakingball)
tidy(rh_bb_model_final) %>% kable(digits = 4)
```
...
rh_breakingball <- rh_breakingball %>% 
  mutate(ideal_prob = (exp(.3093 + 0.0135*ind_vert_break + 0.0006*mean_centered_spinrate)
                       / (1+exp(.3093 + 0.0135*ind_vert_break + 0.0006*mean_centered_spinrate))))
...
lh_bb_model <- glm(pitcher_ideal ~ release_speed + hor_break + 
                     ind_vert_break + mean_centered_spinrate, 
...
rh_ch_model <- glm(pitcher_ideal ~ release_speed + hor_break
                   + ind_vert_break + mean_centered_spinrate, family = "binomial",
                     data = rh_changeup)

tidy(rh_ch_model) %>% kable(digits = 4)
```
...
lh_ch_model <- glm(pitcher_ideal ~ release_speed  + hor_break + ind_vert_break
                     + mean_centered_spinrate, family = "binomial", 
                     data = lh_changeup)
...
rh_ff_model <- glm(pitcher_ideal ~ release_speed  + hor_break + ind_vert_break +
                       release_spin_rate, family = "binomial", 
                     data = rh_fourseam)
...
tidy(rh_ff_model_final) %>% kable(digits = 4)
...
rh_fourseam <- rh_fourseam %>% 
  mutate(ideal_prob = (exp(-0.1858 + ind_vert_break*0.0366)/ (1+exp(-0.1858 + ind_vert_break*0.0366))))
